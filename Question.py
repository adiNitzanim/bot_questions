...
from constants import TEXT_BOX_WIDTH, CHAT_WINDOW_X, \
...
class Question:
    ...
    def __init__(self, num_of_question, question_text, answers, correct_answer):
        self.text_array = from_text_to_array(question_text, TEXT_BOX_WIDTH,
                                             TEXT_FONT_SIZE)
    ...
    def display_answers(self):
        first_answer_y = CHAT_WINDOW_Y + self.question_height + GAP_Y
        for i in range(0, len(self.answers)):
            color = (255, 255, 255)
        ...
        for i in range(0, len(self.answers)):
            answers_buttons.append(
                Button(ANSWER_BOX_X + ((i % 2) * (
                        ANSWER_BOX_WIDTH + GAP_X)),
                       CHAT_WINDOW_Y + self.question_height + GAP_Y + (
                               (i // 2) * (
                               ANSWER_BOX_HEIGHT + (
                               GAP_Y // 2))),
                       ANSWER_BOX_WIDTH,
